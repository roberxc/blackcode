$(document).ready(function(){
	var i=1;
	var p=1;
	$('#agregarTrabajador').click(function(){
		i++;
	$('#dynamic_field').append('<table id="row'+i+'" class="table table-bordered" id="dynamic_field">'+
	'<TR><TH>Rut</TH><TD><input type="text" name="name[]" placeholder="Ingrese" class="form-control" /></TD></TR>'+
	'<TR><TH>Nombre completo</TH><TD><input type="text" name="name[]" placeholder="Ingrese" id="'+i+'" class="form-control" /></TD></TR>'+
	'<TR><TH>Eliminar</TH><TD><button type="button" name="remove" id="'+i+'" class="btn btn-danger btn_remove">X</button></TD></TR> </table>');
	});
                                
	$(document).on('click', '.btn_remove', function(){
		var button_id = $(this).attr("id"); 
		$('#row'+button_id+'').remove();
 	});

	
});